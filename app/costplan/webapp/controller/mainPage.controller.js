sap.ui.define([
    ...
    return Controller.extend("costplan.controller.mainPage", {
                ...
                items: {
                    ...
                    template: new sap.m.ColumnListItem({
                        cells: [
                            ...
                            //     type: "Reject",
                            //     press: this.onDeleteEDLine.bind(this)
                            // })
        ...
        // onDeleteEDLine: async function (oEvent) {
        //     console.log("hereee");
        ...
        //     const sBatchGroupId = "deleteGroup_" + Date.now(); // ✅ important

        //     try {

        //         if (!oRowData.__isNew) {

        //             const oListBinding = oSimModel.bindList(
        //                 "/EngineeringDesignEntry",
        //                 null,
        //                 null,
        //                 [
        //                     new sap.ui.model.Filter("Salesdocument", "EQ", sSalesDoc),
        //                     new sap.ui.model.Filter("ItemNumber", "EQ", sItemNum),
        //                     new sap.ui.model.Filter("description", "EQ", oRowData.description)
        //                 ],
        //                 { $$groupId: sBatchGroupId } // ✅ attach group
        //             );

        //             const aContexts = await oListBinding.requestContexts();

        //             for (const oCtx of aContexts) {
        //                 oCtx.delete(sBatchGroupId); // ✅ pass groupId
        //             }

        //             await oSimModel.submitBatch(sBatchGroupId); // ✅ pass groupId
        //         }

        //         // Remove from UI
        //         const aData = oViewModel.getProperty("/simulationData");
        //         const iIndex = aData.indexOf(oRowData);
        //         aData.splice(iIndex, 1);

        //         oViewModel.setProperty("/simulationData", aData);
        //         oViewModel.refresh(true);

        //         sap.m.MessageToast.show("Line deleted successfully");

        //     } catch (error) {
        //         console.error("Delete failed:", error);
        //         sap.m.MessageBox.error("Delete failed: " + error.message);
        //     }
        // },
        ...
        onSimulationInputChange(oEvent) {
            ...
            //console.log("E and D after input change:", JSON.parse(JSON.stringify(oData)));
            //this.byId("materialTable").getBinding("items").refresh(true); // Force refresh
            this.updateTotalAmount();
        },
        ...
        onSaveSimulation: async function () {
            ...
            switch (sCategory) {
                case "EAndD":
                    console.log(oViewModel.getProperty("/simulationData"));

                    const aEandDAll = oViewModel.getProperty("/simulationData") || [];

                    // ✅ 1. Calculate total from ALL rows
                    totalAmount = aEandDAll.reduce((sum, row) =>
                ...
                case "IndirectCost":
                    const aIndirectAll = oViewModel.getProperty("/indirectCostData") || [];

                    totalAmount = aIndirectAll.reduce((sum, row) =>
                        sum + (parseFloat(row.Total) || 0), 0
                    );
                    aDataToSave = oViewModel.getProperty("/indirectCostData")
                ...
                case "Material":
                    const aMaterialAll = oViewModel.getProperty("/materialData") || [];

                    totalAmount = aMaterialAll.reduce((sum, row) =>
                        sum + (parseFloat(row.Total_Price) || 0), 0
                    );
                    aDataToSave = oViewModel.getProperty("/materialData")
                        .filter(row => row.__isNew)
                        //.filter(row => row)
                        .map(row => {
                ...
                case "Cables":
                    ...
                    const aCablesAll = oViewModel.getProperty("/cablesData") || [];

                    totalAmount = aCablesAll.reduce((sum, row) =>
                        sum + (parseFloat(row.Total_Price) || 0), 0
                    );
                    aDataToSave = oViewModel.getProperty("/cablesData")
                        .filter(row => row.__isNew)
                        //.filter(row => row)
                        .map(row => {
                            // totalAmount += parseFloat(row.Total_Price) || 0;
